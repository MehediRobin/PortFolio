...
export function Project({ title, description, tags, imageUrl }: ProjectProps) {
  ...
  return (
    <motion.div
      ...
      className="group bg-gray-100 max-w-[42rem] border border-black/5 overflow-hidden relative  h-[20rem] mb-6 hover:bg-gray-500"
    >
      <div className="pt-4 pb-7 px-5 max-w-[50%] h-full flex flex-col group-even:ml-[20rem]">
        ...
        <ul className="flex flex-wrap gap-2 mt-auto">
          {tags.map((tag, index) => (
            <li
              className="bg-gray-500 px-3 py-1 text-[0.7rem] uppercase tracking-wider text-white rounded-full"
              key={index}
            >
              {tag}
            </li>
